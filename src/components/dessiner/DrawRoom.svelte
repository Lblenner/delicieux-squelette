<script lang="ts">
	import { request, currentDessin, selectedRoom, complete } from '../../components/appState';
...
<div class="flex flex-col items-center justify-center p-4 gap-2">
	{#if error}
		<div class="w-full card bg-error-200-700-token text-error-800-100-token p-4">
			{error}
		</div>
	{/if}
	{#if $selectedRoom}
		...
		{:then}
			{#if dess && pixel_size}
				<label class="label w-[200px] text-center px-2" on:touchmove|preventDefault for="slide">
				...
				<button class="btn variant-filled p-4 m-4 btn-lg" on:click={comp}>Envoyer</button>
			{:else}
				<div class="flex flex-col items-center p-4 gap-2 lg:max-w-lg max-w-xs">
					<p class="text-center whitespace-pre-line">
						Dessin selectionné : <span class="text-token">{$selectedRoom.name}</span>

						Une fois que le dessin est demandé, l'emplacement est reservé pendant 15 minutes.
					</p>
					{#if $selectedRoom.rules}
						<p class="text-center whitespace-pre-line">Description de la salle :</p>
	...
	{:else}
		<p class="text-center lg:max-w-lg max-w-xs">
			Pas de dessin selectionné ! Clique sur le lien du dessin ou rejoins un dessin public pour commencer
		</p>
	{/if}
</div>
