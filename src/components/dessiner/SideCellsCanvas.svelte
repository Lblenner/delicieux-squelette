<script lang="ts">
	...
	$: drawBuffer = (current_dessin: Dessin, image_resolution: number, pixel_size: number) => {
		...
		current_dessin.side_cells.forEach((cell) => {
			...
			let imageData = new ImageData(image_resolution * pixel_size, image_resolution * pixel_size);

			let content = toBuffer(cell.content, image_resolution, pixel_size);

			imageData.data.set(content);

			let x = dxy + image_resolution * pixel_size * (cell.x - current_dessin.selected_cell.x);
			let y = dxy + image_resolution * pixel_size * (cell.y - current_dessin.selected_cell.y);
		...
		content.forEach((_, content_index) => {
			...
			for (let i = 0; i < pix_size; i++) {
				for (let j = 0; j < pix_size; j++) {
					let bufferPos = (x * pix_size + i + (j + y * pix_size) * res * pix_size) * 4;
...
<div class="card p-4">
	<div style="width: {w}px; height: {h}px">
		<div class="relative bg-slate-500/[0.7]">
			<canvas bind:this={canvas} width={w} height={h} style="width: {w}px; height: {h}px;" />
