...
import { ToolSelector, STROKE_TYPE } from '@/components/tool_selector';
import { useState, useRef } from 'react';
import { BORDER, DEFAULT_ROOM, HEIGHT, MYURL, WIDTH, lovelo, requestOptions } from '@/components/consts';
...
export default function Voir() {
  ...
  let [strokeSize, setStrokeSize] = useState(0);
  ...
  async function requestAction() {
    setState("loading")
    if (await requestDraw()) {
      dessinRef.current = Array(WIDTH * HEIGHT).fill(0)
      setState("loaded")
      return
    }
    setState("idle")
  ...
  switch (state) {
    ...
    case "loaded":
      ...
      break;
    default:
      content = <div>errora muco importante</div>
  }
